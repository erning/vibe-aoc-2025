...
## 1. Setup Input Files
- [ ] 1.1 Fetch puzzle description from https://adventofcode.com/2025/day/1
- [ ] 1.2 Extract example data from puzzle and save to `inputs/01-example.txt`
- [ ] 1.3 Download real input via `./scripts/download-input.sh 1` to `inputs/01-input.txt`

## 2. Implement Part One
- [ ] 2.1 Create `src/day01.rs` with module documentation following project conventions
...
- [ ] 2.5 Update `src/lib.rs` to declare `day01` module (if needed)
- [ ] 2.6 Update `src/main.rs` to register day01 puzzle (if needed)
- [ ] 2.7 Run `cargo test day01` to verify example passes
- [ ] 2.8 Run with real input: `cargo run --release -- 1`
- [ ] 2.9 Submit Part 1 answer via `./scripts/submit-answer.sh 1 1 <answer>` (if unknown)
...
- [ ] 3.4 Run `cargo test day01` to verify tests pass
- [ ] 3.5 Run with real input: `cargo run --release -- 1`
- [ ] 3.6 Submit Part 2 answer via `./scripts/submit-answer.sh 1 2 <answer>` (if unknown)
